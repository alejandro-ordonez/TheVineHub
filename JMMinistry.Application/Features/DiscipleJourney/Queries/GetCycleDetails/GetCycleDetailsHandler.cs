using JMMinistry.Application.Services;
using JMMinistry.Common.Dtos.DiscipleJourney;
using JMMinistry.Common.Dtos.DiscipleJourney.Enums;
using Mediator;
using Npgsql;

namespace JMMinistry.Application.Features.DiscipleJourney.Queries.GetCycleDetails;

public class GetCycleDetailsHandler(IJmDbContext dbContext)
    : IQueryHandler<GetCycleDetailsQuery, IList<CycleEnrollmentDto>>
{
    public async ValueTask<IList<CycleEnrollmentDto>> Handle(GetCycleDetailsQuery request, CancellationToken cancellationToken)
    {
        var cycleParam = new NpgsqlParameter("p_cycle", request.CycleId);

        var rows = await dbContext.ExecuteTableFunctionAsync<CycleDetailRow>(
            "SELECT * FROM get_cycle_details(@p_cycle)",
            cancellationToken,
            cycleParam);

        return rows.Select(r => new CycleEnrollmentDto
        {
            Id = r.enrollment_id,
            DiscipleId = r.disciple_id,
            DiscipleName = r.disciple_name,
            CycleStaffId = r.cycle_staff_id,
            GuideName = r.guide_name,
            Status = (StepStatus)r.status,
            EnrolledAt = r.enrolled_at,
            AttendanceCount = r.attendance_count
        }).ToList();
    }
}
