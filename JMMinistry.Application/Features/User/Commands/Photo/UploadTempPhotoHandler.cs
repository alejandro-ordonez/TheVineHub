...
{
    public async ValueTask<string> Handle(UploadTempPhotoCommand request, CancellationToken cancellationToken)
    {
        return await photoService.SaveTempPhotoAsync(request.ImageStream);
    }
}
