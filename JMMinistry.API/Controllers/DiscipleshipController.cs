...
namespace JMMinistry.API.Controllers
{
    ...
    public class DiscipleshipController(IMediator mediator) : ControllerBase
    {
        ...
        [HttpGet("{discipleId}/notes/{noteId:int}")]
        public async Task<ActionResult<DiscipleshipNoteDto>> GetNoteById(string discipleId, int noteId)
        {
            var requestorId = HttpContext.GetDocumentClaim() ?? throw new MissingInTokenException();
        ...
        [HttpGet("{discipleId}/notes/{noteId:int}/entries")]
        public async Task<ActionResult<IList<DiscipleshipNoteEntryDto>>> GetNoteEntries(string discipleId, int noteId)
        {
            var requestorId = HttpContext.GetDocumentClaim() ?? throw new MissingInTokenException();
        ...
        [HttpPost("{discipleId}/notes/{noteId:int}/entries")]
        public async Task<ActionResult<DiscipleshipNoteEntryDto>> CreateNoteEntry(string discipleId, int noteId, [FromBody] CreateDiscipleshipNoteEntryDto dto)
        {
            var requestorId = HttpContext.GetDocumentClaim() ?? throw new MissingInTokenException();
